...
{
	//utworzenie nowego w�z�a, ustawiamy prev na nullptr, poniewa� pierwszy element listy nie ma poprzednika
...
void DoublyLinkedList::insertBack(int value) // wstaw wartość na koniec
{
	Node* newNode = new Node();
	newNode->data = value;
	...
	_tail = newNode;
	newNode->next = nullptr;
	// kontrola czy jest dostęp do nowego elementu - test
...
void DoublyLinkedList::print() const
{
	...
	while (tmp != nullptr) //przechodzenie listy - odwiedzanie wszytkich wez��w (ang. traverse)
	{
		std::cout << tmp->data << "<->";
		tmp = tmp->next;
	}
...
void DoublyLinkedList::insertAfter(int value, const unsigned int index)
{
	...
	while (tmp != nullptr && currentIndex != index) //przechodzenie listy - odwiedzanie wszytkich wez��w (ang. traverse)
	{
		++currentIndex;
		tmp = tmp->next;
	}

	if (tmp != nullptr && tmp->next != nullptr) //sprawdzenie czy mamy przynajmniej dwa elementy w liscie, aby wstawi� nasz nowy element pomi�dzy te istniej�ce
	{
		Node* newNode = new Node();
		newNode->data = value;

		//Node* nodeAfterNewOne = tmp->next; //element kt�ry znajduje si� na naszym nowo wstawionym elementem
		//nodeAfterNewOne->next = newNode;

		tmp->next->prev = newNode; //ustawienie na nasz nowy element pola prev z nast�pnego elementu
		...
		newNode->next = tmp->next; //nowy element wskazuje na element nast�pny
...
void DoublyLinkedList::removeValue(int value)
{
}
